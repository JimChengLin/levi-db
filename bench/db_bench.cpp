...
void leveldb_write_bench() {
    const std::string db_name = "/tmp/level_bench_db";
    if (levidb8::env_io::fileExists(db_name)) {
    ...
    options.create_if_missing = true;
    auto s = leveldb::DB::Open(options, db_name, &db);
    assert(s.ok());
...
void leveldb_read_bench() {
    const std::string db_name = "/tmp/level_bench_db";
    leveldb::DB * db;
    auto s = leveldb::DB::Open({}, db_name, &db);
    assert(s.ok());
...
void write_bench() {
    ...
    PRINT_TIME(leveldb_write_bench);
}
